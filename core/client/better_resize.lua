...
--- Improved mouse-resize handler for tiled layouts.
-- Replaces `awful.layout.suit.tile.mouse_resize_handler` and its
-- bottom-variant so that resizing adjusts both the master-width factor
-- and per-column window factors via mouse drag.
-- @module core.client.better_resize
local capi = {
    client = client,
    mouse = mouse,
    screen = screen,
...
--- Mouse-resize handler for tiled layouts.
-- Adjusts `master_width_factor` (horizontal drag near the split) and
-- per-column window factors (vertical drag near inter-client borders).
-- Called as a replacement for the default tile-resize handler.
-- @tparam function m Coordinate mapper. Identity for `tile`,
--   {x=y, y=x} swapper for `tile.bottom`.
-- @tparam client c The client being resized at its edge
-- @treturn boolean True if the grab started, false otherwise
local function mouse_resize_handler(m, c)
    ...
    do
        ...
        local mw_split = wa.x
            + wa.width * c.screen.selected_tag.master_width_factor
