--- Tag (workspace) core.
-- Defines the per-screen tag list (`A W E S O M E W M`) and the default
-- layout rotation. The first layout in the rotation (`awful.layout.suit.max`)
-- is what each new tag starts with; the rest are reachable via Mod4+Space.
-- @module core.tag
...
capi.tag.connect_signal("request::default_layouts", function()
    awful.layout.append_default_layouts({
        ...
        awful.layout.suit.max, -- Maximized (default layout for all tags)
...
awful.screen.connect_for_each_screen(function(s)
    -- Create 9 named tags per screen, using the first layout as default
    awful.tag(
        { "A", "W", "E", "S", "O", "M", "E", "W", "M" },
        s,
