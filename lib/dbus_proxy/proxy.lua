...
local string = string
local table = table
local unpack = table.unpack or unpack -- luacheck: globals unpack (compatibility with Lua 5.1)
...
local function build_args(method, ...)
    ...
    assert(
        #{ ... } == #args,
        string.format("Expected %d parameters but got %d", #args, #{ ... })
    )
...
local function generate_accessor(property)
    ...
    if property.flags.READABLE then
        ...
        accessor.getter = function()
            gdebug.print_warning(
                string.format("Property '%s' is not readable", property.name)
            )
    ...
    if property.flags.WRITABLE then
        ...
        accessor.setter = function()
            gdebug.print_warning(
                string.format("Property '%s' is not writable", property.name)
            )
...
local function generate_fields(proxy)
    ...
    if not xml_data_str then
        ...
        -- error(string.format(
        -- "Failed to introspect object '%s'\nerror: %s\ncode: %s",
        -- proxy.name, err or "<unknown>", (err and err.code) or "<unknown>"))
        return
    end
    ...
    for _, iface in ipairs(node.interfaces) do
        for _, method in ipairs(iface.methods) do
            if not proxy[method.name] then
                ...
                proxy[method.name .. "Async"] =
                    generate_async_method(iface.name, method)
            else
    ...
    for k, _ in pairs(proxy) do
        if proxy.accessors[k] ~= nil then
            ...
            if proxy.accessors[new_name] == nil then
                proxy.accessors[new_name], proxy.accessors[k] =
                    proxy.accessors[k], nil
            end
        end
    end
end
