...
function audio:get_default_sink_data(callback)
    ...
    awful.spawn.easy_async_with_shell(
        "LANG=C pactl get-sink-mute @DEFAULT_SINK@",
        function(stdout)
...
function audio:get_default_source_data()
    awful.spawn.easy_async_with_shell(
        "LANG=C pactl get-source-volume @DEFAULT_SOURCE@",
        function(stdout)
            self.default_source_volume = tonumber(stdout:match("/%s+(%d+)%%"))
            self:emit_signal(
                "default-source::volume",
                self.default_source_volume
            )
        end
    )

    awful.spawn.easy_async_with_shell(
        "LANG=C pactl get-source-mute @DEFAULT_SOURCE@",
        function(stdout)
