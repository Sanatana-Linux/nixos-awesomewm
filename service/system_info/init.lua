...
function system_info:_update_cpu_usage()
    ...
    spawn.easy_async("cat /proc/stat", function(stdout)
        ...
        if total_diff > 0 then
            wp.cpu_usage =
                math.floor((total_diff - idle_diff) * 100 / total_diff)
...
function system_info:_update_memory_usage()
    ...
    spawn.easy_async("cat /proc/meminfo", function(stdout)
        ...
        if mem_total and mem_available then
            ...
            local ram_used = wp.ram_total
                - math.floor(tonumber(mem_available) / 1024)
        ...
        if swap_total and swap_free then
            ...
            local swap_used = wp.swap_total
                - math.floor(tonumber(swap_free) / 1024)
            wp.swap_usage = wp.swap_total > 0
                    and math.floor(swap_used * 100 / wp.swap_total)
                or 0
            self:emit_signal(
                "property::swap_usage",
                wp.swap_usage,
...
function system_info:_update_disk_usage()
    ...
    spawn.easy_async("df -h /", function(stdout)
        ...
        local size, used, available, usage_percent =
            disk_line:match("%S+%s+(%S+)%s+(%S+)%s+(%S+)%s+(%d+)%%")
        ...
        function(stdout)
            ...
            spawn.easy_async(
                ...
                function(stdout_amd)
                    ...
                    spawn.easy_async(
                        "timeout 1s intel_gpu_top -J -s 1000",
                        function(stdout_intel)
                            -- Parse JSON output for render/3d usage
                            local gpu_percent_intel = stdout_intel:match(
                                '"Render/3D".-"busy":([%d%.]+)'
                            )
                            if gpu_percent_intel then
                                wp.gpu_usage =
                                    math.floor(tonumber(gpu_percent_intel))
                                self:emit_signal(
                                    "property::gpu_usage",
                                    wp.gpu_usage
                                )
