...
function screenshot:take(args)
	...
	local self_ref = self
	local folder = user.screenshots_folder or os.getenv("HOME") .. "/Pictures"
	...
	awful.spawn.easy_async_with_shell(cmd, function(stdout, stderr, reason, exit_code)
		...
		if exit_code == 0 and file_exists(outpath) then
			...
			naughty.notification {
				...
				message  = "Could not take screenshot. Error: " .. (stderr or "Unknown error"),
				timeout  = 0
			}
...
function screenshot:annotate(path)
	...
	local cmd = string.format("satty --filename '%s' --fullscreen --output-filename '%s'", path, path)
