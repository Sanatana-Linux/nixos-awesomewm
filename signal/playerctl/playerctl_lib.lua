...
function playerctl:play_pause(player)
    player = player or self._private.manager.players[1]
    if player then player:play_pause() end
end

function playerctl:previous(player)
    player = player or self._private.manager.players[1]
    if player then player:previous() end
end

function playerctl:next(player)
    player = player or self._private.manager.players[1]
    if player then player:next() end
end

function playerctl:set_loop_status(loop_status, player)
    player = player or self._private.manager.players[1]
    if player then player:set_loop_status(loop_status) end
end
...
function playerctl:set_position(position, player)
    player = player or self._private.manager.players[1]
    if player then player:set_position(position * 1000000) end
end

function playerctl:set_shuffle(shuffle, player)
    player = player or self._private.manager.players[1]
    if player then player:set_shuffle(shuffle) end
end

function playerctl:cycle_shuffle(player)
    player = player or self._private.manager.players[1]
    if player then player:set_shuffle(not player.shuffle) end
end

function playerctl:set_volume(volume, player)
    player = player or self._private.manager.players[1]
    if player then player:set_volume(volume) end
end

function playerctl:get_manager() return self._private.manager end

function playerctl:get_active_player() return self._private.manager.players[1] end
...
local function emit_metadata_signal(
    ...
    if artUrl ~= '' then
        ...
        utilties.filesystem.save_image_async_curl(artUrl, art_path, function()
            self:emit_signal(
                'metadata',
                title,
                artist,
...
local function name_is_selected(self, name)
    if self.ignore[name.name] then return false end

    if #self.priority > 0 then
        for _, arg in pairs(self.priority) do
            if arg == name.name or arg == '%any' then return true end
        end
        return false
    end
...
local function init_player(self, name)
    if name_is_selected(self, name) then
        ...
        player.on_exit = function(player, shuffle_status) exit_cb(self, player) end
...
local function player_compare_name(self, name_a, name_b)
    ...
    if name_a == name_b then return 0 end
...
local function start_manager(self)
    ...
    if #self.priority > 0 then
        self._private.manager:set_sort_func(
            function(a, b) return player_compare(self, a, b) end
        )
    end
    ...
    self._private.position_timer = gtimer({
        ...
        callback = function() position_cb(self) end,
    })
...
local function new(args)
    ...
    gtimer.delayed_call(function() start_manager(ret) end)

    return ret
end

function playerctl.mt:__call(...) return new(...) end
