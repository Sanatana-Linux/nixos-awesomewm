            ...
            {
                {
                    image = gears.surface.crop_surface({
                        ...
                        ratio = s.geometry.width
                            / (s.geometry.height - dpi(40)),
                    }),
                    widget = wibox.widget.imagebox,
                },
                {
                    {
                        {
                            {
                                helpers.colorizeText({ text = "Empty" }),
                                top = dpi(5),
                                bottom = dpi(5),
                                left = dpi(8),
                                right = dpi(8),
...
local previewbox = wibox({
    ...
    visible = false,
    widget = wibox.container.background,
    background = beautiful.bg_gradient,
    shape = helpers.rrect(),
})
...
awesome.connect_signal("widget::preview", function()
    ...
    for i, tag in ipairs(tags) do
        ...
        local preview = wibox.widget({
            helpers.addHover(createpreview(tag, tag.screen, geometry)),
            buttons = {
