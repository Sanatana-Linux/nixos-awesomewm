...
local function build_hotkeys(entries)
    ...
    for _, entry in ipairs(entries) do
        ...
        if info and info.description then
            ...
            if not groups[gn] then groups[gn] = {} end
            ...
            for _, b in ipairs(groups[gn]) do
                ...
                if match then
                    for i, m in ipairs(mods) do
                        if b.modifiers[i] ~= m then match = false; break end
                    end
                end
                if match then block = b; break end
            end
            if not block then
...
if grid.keys and grid.keys.move then
    ...
    for g, b in pairs(grid_resize) do
        if not grid_groups[g] then grid_groups[g] = {} end
        for _, v in ipairs(b) do table.insert(grid_groups[g], v) end
    end
...
if map.keys and map.keys.layout then
    ...
    for g, b in pairs(map_resize) do
        if not map_groups[g] then map_groups[g] = {} end
        for _, v in ipairs(b) do table.insert(map_groups[g], v) end
    end
