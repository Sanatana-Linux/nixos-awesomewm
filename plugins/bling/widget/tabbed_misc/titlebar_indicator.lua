...
local tabbed_module =
    require(tostring(...):match('.*bling') .. '.module.tabbed')
...
return function(c, opts)
    ...
    awesome.connect_signal(
        ...
        function(_, removed_c)
            ...
            for idx, icon in ipairs(tabbed_icons.children) do
                if icon._client == removed_c then tabbed_icons:remove(idx) end
            end

            -- Empty list
            if removed_c == c then tabbed_icons:reset() end
        end
    )
    ...
    local function recreate(group)
        if tbl_contains(group.clients, c) then
            ...
            -- Autohide?
            if #group.clients == 1 then return end
            ...
            for idx, client in ipairs(group.clients) do
                ...
                if
                    ...
                    and opts.widget_template.update_callback
                then
                ...
                for _, w in ipairs(widget:get_children_by_id('bg_role')) do
                    w:add_button(
                        awful.button(
                            {},
                            1,
                            function() tabbed_module.switch_to(group, idx) end
                        )
                    )
