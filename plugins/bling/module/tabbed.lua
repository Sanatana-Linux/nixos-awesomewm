...
tabbed.add = function(c, tabobj)
    if c.bling_tabbed then tabbed.remove(c) end
...
tabbed.update = function(tabobj)
    ...
    if not beautiful.tabbar_disable then tabbed.update_tabbar(tabobj) end
end
...
tabbed.update_tabbar = function(tabobj)
    ...
    for idx, c in ipairs(tabobj.clients) do
        local buttons = gears.table.join(
            awful.button({}, 1, function() tabbed.switch_to(tabobj, idx) end)
        )
