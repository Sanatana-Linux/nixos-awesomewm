...
local function get_icon_by_pid_command(self, client, apps)
	...
	if pid ~= nil then
		...
		for _, app in ipairs(apps) do
			...
			if executable and executable:find(pid_command, 1, true) then return self:get_gicon_path(app:get_icon()) end
		end
	end
end
...
local function get_icon_by_icon_name(self, client, apps)
	...
	if icon_name ~= nil then
		for _, app in ipairs(apps) do
			...
			if name and name:find(icon_name, 1, true) then return self:get_gicon_path(app:get_icon()) end
		end
	end
end
	...
	if client.class ~= nil then
		...
		for _, app in ipairs(apps) do
			...
			for _, possible_icon_name in ipairs(possible_icon_names) do
				if id and id:find(possible_icon_name, 1, true) then return self:get_gicon_path(app:get_icon()) end
			end
		end
	end
end
