...
local subscribable = require("modules.effects.subscribable")
...
-- @see timed
local function simulate_easing(
    pos,
    duration,
    intro,
    intro_e,
...
--function creates timers which run in the background handling effects at distinct rates
...
local function create_timeout(rate, override_dt)
    ...
    return glib.timeout_add(glib.PRIORITY_DEFAULT, initial_dt * 1000, function()
        ...
        for _, obj in pairs(MANAGER.timeds) do
            if
                obj.rate == rate
                and obj.override_dt == override_dt
                and obj._time ~= obj.duration
                and not obj.pause
            then
                --increment time
                ...
                obj._dx = get_dx(
                    obj._time,
                    obj.duration,
                    (obj._is_inter and obj.inter or obj.intro)
                        * (obj.prop_intro and obj.duration or 1),
                    obj._is_inter and obj.easing_inter.easing
                        or obj.easing.easing,
...
local function timed(args)
    ...
    function obj:reset_values()
        ...
        self.intro = args.intro
            or (
                MANAGER.timed.defaults.intro > self.duration * 0.5
        ...
        if
            self.intro > (self.prop_intro and 0.5 or self.duration)
            and not args.outro
        then
            self.outro = math.max(
                (self.prop_intro and 1 or self.duration - self.intro),
                0
            )
        ...
        self.awestore_compat = args.awestore_compat
            or MANAGER.timed.defaults.awestore_compat

        --animation logic changes
        self.override_simulate = args.override_simulate
            or MANAGER.timed.defaults.override_simulate
        self.rapid_set = args.rapid_set == nil and self.awestore_compat
            or args.rapid_set
        self.clamp_position = args.clamp_position
            or MANAGER.timed.defaults.clamp_position
        ...
        self._props = {
            ...
            override_dt = args.override_dt
                or MANAGER.timed.defaults.override_dt,
        }
    end
    obj:reset_values()
    ...
    local function set(value)
        ...
        if not TIMEOUTS[obj._dt_index][obj.rate] then
            TIMEOUTS[obj._dt_index][obj.rate] =
                create_timeout(obj.rate, obj.override_dt)
        end
        obj._dt = 1 / obj.rate
        ...
        if
            obj.override_simulate
            or (
                obj._b ~= 0
                and obj._b / math.abs(obj._b) ~= obj._m / math.abs(obj._m)
            )
        then
