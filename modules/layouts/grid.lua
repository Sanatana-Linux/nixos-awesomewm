...
local function get_rail(c)
    ...
    local rail =
        { x = { wa.x, wa.x + wa.width }, y = { wa.y, wa.y + wa.height } }
...
grid.resize_to = function(dir, is_rail, is_reverse)
    ...
    if dir == "up" then
        if is_rail then
            ...
            local f, l, s = unpack(
                is_reverse and { 1, #data.rail.y, 1 } or { #data.rail.y, 1, -1 }
            )
            for i = f, l, s do
                if
                    is_reverse and data.rail.y[i] > g.y
                    or not is_reverse and data.rail.y[i] < g.y
                then
                    ng = {
                        y = data.rail.y[i],
                        height = g.height + g.y - data.rail.y[i],
                    }
                    break
                end
            end
            ...
            ng = {
                y = g.y - sign * data.cell.y,
                height = g.height + sign * data.cell.y,
            }
        end
    elseif dir == "down" then
        if is_rail then
            local f, l, s = unpack(
                is_reverse and { #data.rail.y, 1, -1 } or { 1, #data.rail.y, 1 }
            )
            for i = f, l, s do
                if
                    is_reverse and data.rail.y[i] < (g.y + g.height - 1)
                    or not is_reverse
                        and data.rail.y[i] > (g.y + g.height + 1)
                then
    ...
    elseif dir == "left" then
        if is_rail then
            local f, l, s = unpack(
                is_reverse and { 1, #data.rail.x, 1 } or { #data.rail.x, 1, -1 }
            )
            for i = f, l, s do
                if
                    is_reverse and data.rail.x[i] > g.x
                    or not is_reverse and data.rail.x[i] < g.x
                then
                    ng = {
                        x = data.rail.x[i],
                        width = g.width + g.x - data.rail.x[i],
                    }
                    break
                end
            end
            ...
            ng = {
                x = g.x - sign * data.cell.x,
                width = g.width + sign * data.cell.x,
            }
        end
    elseif dir == "right" then
        if is_rail then
            local f, l, s = unpack(
                is_reverse and { #data.rail.x, 1, -1 } or { 1, #data.rail.x, 1 }
            )
            for i = f, l, s do
                if
                    is_reverse and data.rail.x[i] < (g.x + g.width)
                    or not is_reverse
                        and data.rail.x[i] > (g.x + g.width + 1)
                then
...
function grid.arrange(p)
    ...
    grid.data.cell = make_cell({
        width = wa.width + 2 * p.useless_gap,
        height = wa.height + 2 * p.useless_gap,
    }, cellnum)
...
function grid.move_handler(c, _, hints)
    ...
    if is_diff(hg, g, grid.data.cell) then
        fullgeometry(c, fit_cell(hg, grid.data.cell))
    end
end
        ...
        for _, v in ipairs(_mouse.buttons) do
            if v then
                ...
                if is_diff(ng, cg, grid.data.cell) then
                    cg = fullgeometry(c, fit_cell(ng, grid.data.cell))
                end
