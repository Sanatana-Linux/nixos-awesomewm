...
local function nav_hilight(target)
    local nav = utils.get_navigator()
    if nav and nav.hilight then
        nav.hilight.show(target)
    end
end
...
function map.construct_itempack(cls, wa, is_vertical, parent)
    ...
    function pack:incfacror(index, df, vertical)
        if vertical == self.is_vertical then
            self.items[index].factor =
                math.max(self.items[index].factor + df, 0.1)
...
local function construct_tree(wa, t)
    ...
    ------------------------------------------------------------
    local tree = map.scheme[t] and map.scheme[t].construct(wa)
        or map.base_construct(wa)
    ...
    function tree:create_group(c, is_vertical)
        ...
        notify(
            "New " .. (is_vertical and "vertical" or "horizontal") .. " group"
        )
    end
    ...
    function tree:insert_group(is_vertical)
        ...
        local new_pack =
            map.construct_itempack({}, pack.wa, is_vertical, pack.parent)
...
function map.new_group(is_vertical)
    ...
    hitimer:connect_signal("timeout", function()
        nav_hilight(map.data[t].set[map.data[t].active].wa)
        hitimer:stop()
        hitimer = nil
    end)
...
function map.move_group(dn)
    ...
    if pack.parent then
        ...
        if pack.parent.items[i + dn] then
            pack.parent.items[i], pack.parent.items[i + dn] =
                pack.parent.items[i + dn], pack.parent.items[i]
            t:emit_signal("property::layout")
        end
    end
end
...
function map.base_construct(wa)
    local tree =
        { set = {}, active = 1, autoaim = map.base_autoaim, aim = map.base_aim }
