...
local function new(opts)
    ...
    local ret = wibox.widget({
        ...
        bg = color_alpha(beautiful.bg_alt, "aa"),
        fg = beautiful.fg,
    ...
    -- State update: toggle UI between active/inactive styles
    --- @tparam boolean is_active
    -- @local
    local function update_ui(is_active)
        wp.is_active = is_active
        if is_active then
            ret:set_bg(color_alpha(beautiful.bg_alt, "aa"))
            ret:set_fg(beautiful.fg)
            ret:set_border_color(beautiful.fg_alt)
            toggle_button:set_bg(color_alpha(beautiful.bg_alt, "aa"))
    ...
    -- Hover effect handler: dim toggle area background on hover
    --- @tparam boolean is_hovered
    -- @local
    local function on_hovered(is_hovered)
        ...
        if is_hovered then
            toggle_button:set_bg(color_alpha(beautiful.bg_urg, "aa"))
            separator:set_color(beautiful.fg_alt)
        else
            toggle_button:set_bg(nil)
            separator:set_color(beautiful.fg_alt)
    ...
    -- Press effect handler: flash toggle area on click
    --- @tparam boolean is_pressed
    -- @local
    ...
    reveal_button:connect_signal("mouse::enter", function()
        if not wp.is_active then
            reveal_button:set_bg(color_alpha(beautiful.bg_urg, "aa"))
            reveal_icon:set_image(
                gcolor.recolor_image(arrow_icon, beautiful.fg)
            )
        end
