...
function styled_button.create(args)
    ...
    local outer_container = wibox.widget({
        ...
        shape = button_shape,
        border_width = dpi(1),
        border_color = args.selected and (color_alpha(beautiful.fg, "66"))
            or "transparent",
        bg = args.selected and beautiful.bg_gradient_button_alt
            or beautiful.bg_gradient_button,
        widget = wibox.container.background,
    ...
    function outer_container:set_selected(selected)
        if selected then
            ...
            outer_container.border_color = color_alpha(beautiful.fg, "66")
        else
    ...
    --- Revert button to its selected/unselected state after a hover ends.
    -- @local
...
--- Build an icon-only button widget.
-- @tparam[opt] table args
--   @tparam[opt] string args.icon Icon path
--   @tparam[opt] number args.icon_size Icon dimension (default dpi(32))
--   @tparam[opt] number args.padding Inner padding (default dpi(3))
--   @tparam[opt] table args.buttons awful.button bindings
-- @treturn table A wibox widget
function styled_button.create_icon_button(args)
    ...
    btn:connect_signal("mouse::enter", function()
        ...
        btn.border_color = color_alpha(beautiful.fg, "66")
    end)
...
--- Build a text-only styled button.
-- Convenience wrapper around `styled_button.create` for text labels.
-- @tparam[opt] table args Same as `styled_button.create` plus `text` and `font`.
--   @tparam string args.text Button label text
--   @tparam[opt] string args.font Font string
-- @treturn table A wibox widget
