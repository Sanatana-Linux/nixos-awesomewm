...
local window_switcher_hide = function(window_switcher_box)
	...
	for _, c in pairs(window_switcher_minimized_clients) do
		if c and c.valid and not (client.focus and client.focus == c) then c.minimized = true end
	end
	-- Reset helper table
...
local enable = function(opts)
	...
	local mouse_keys = gears.table.join(
		...
		awful.button({
			...
			on_press = function() cycleClientsByIdx(-1) end,
		}),
	...
	local keyboard_keys = {
		...
		[unminimize_key] = function()
			if awful.client.restore() then client.focus = awful.client.restore() end
		end,
		[kill_client_key] = function()
			if client.focus then client.focus:kill() end
		end,

		[cycle_key] = function() cycleClientsByIdx(1) end,

		[previous_key] = function() cycleClientsByIdx(1) end,
		[next_key] = function() cycleClientsByIdx(-1) end,

		[vim_previous_key] = function() cycleClientsByIdx(1) end,
		[vim_next_key] = function() cycleClientsByIdx(-1) end,
	}

	window_switcher_box:connect_signal('property::width', function()
		if window_switcher_box.visible and get_num_clients() == 0 then window_switcher_hide(window_switcher_box) end
	end)
