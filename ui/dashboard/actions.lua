...
local function make_button(options)
	...
	local button = wibox.widget({
		...
		set_icon = function(self, icon) self:get_children_by_id('icon_role')[1].markup = icon end,
	})

	awesome.connect_signal(signal, function(payload) signal_callback(button, payload) end)
...
local network = make_button({
	default_icon = '睊',
	callback = function() awful.spawn('bash ' .. gfs.get_configuration_dir() .. 'scripts/toggle-network.sh') end,
	signal = {
...
local volume = make_button({
	default_icon = '',
	callback = function() awful.spawn('pamixer -t && pamixer --get-mute') end,
	signal = {
		name = 'signal::volume',
