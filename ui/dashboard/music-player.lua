...
local artist = wibox.widget({
    markup = "No artist",
    font = beautiful.font_name .. " " .. tostring(
        tonumber(beautiful.font_size) - 1
    ),
    widget = wibox.widget.textbox,
})
...
playerctl:connect_signal(
    "metadata",
    function(_, music_title, music_artist, music_album_path)
        title:set_markup_silently(
            "<b>"
                .. utilities.textual.limit_by_length(music_title, 16, true)
                .. "</b>"
        )
        artist:set_markup_silently(
            "By " .. utilities.textual.limit_by_length(music_artist, 19, true)
        )
