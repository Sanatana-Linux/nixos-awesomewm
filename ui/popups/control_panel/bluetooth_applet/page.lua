...
local function create_dev_widget(path)
    ...
    connect_button.shape = shapes.rrect(10)
    ...
    pair_button.shape = shapes.rrect(10)
    pair_button:buttons({
        awful.button({}, 1, function()
    ...
    trust_button.shape = shapes.rrect(10)
    ...
    dev:connect_signal("property::percentage", function(_, perc)
        percentage:set_markup(
            perc ~= nil and string.format("%.0f%%", perc) or ""
        )
    end)
    ...
    name:set_markup(
        (dev:get_connected() and text_icons.check .. " " or "")
            .. (dev:get_name() or dev:get_address())
    )
    percentage:set_markup(
        dev:get_percentage() and string.format("%.0f%%", dev:get_percentage())
            or ""
    )
...
local function on_discovering(self, discovering)
    ...
    bottombar_discover_button:set_fg_normal(
        discovering and beautiful.fg_alt or beautiful.fg
    )
    bottombar_discover_button:set_bg_hover(
        discovering and beautiful.fg_alt or beautiful.ac
    )
    bottombar_discover_button:set_fg(
        discovering and beautiful.fg_alt or beautiful.fg
    )
