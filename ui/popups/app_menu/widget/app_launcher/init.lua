...
local function search(self, text)
  ...
  self._private.apps_per_page =
    math.min(#self._private.matched_entries, self._private.max_apps_per_page)
...
local function page_backward(self, direction)
  ...
  elseif
    ...
    and #self._private.matched_entries >= self._private.max_apps_per_page
  then
  ...
  elseif self.wrap_page_scrolling then
    if direction == "up" then
      select_app(
        self,
        math.min(rows, #self._private.grid.children % 9),
        columns
      )
    else
      -- Keep the same row from last page
      select_app(
        self,
        math.min(pos.row, #self._private.grid.children % 9),
        columns
      )
    end
  end
...
local function page_forward(self, direction)
  ...
  if self._private.current_page > 1 or self.wrap_page_scrolling then
    if direction == "down" then
      ...
      local last_col_max_row =
        math.min(pos.row, #self._private.grid.children % self.apps_per_row)
...
function app_launcher:show()
  ...
  slide_right.target = screen.geometry.height
    - (screen.app_launcher.height + beautiful.useless_gap * 2)
...
local function new(args)
  ...
  args.hide_on_right_clicked_outside = args.hide_on_right_clicked_outside == nil
      and true
  ...
  ret._private.exit_button:connect_signal(
    "button::press",
    function(c, _, _, button)
      if button == 1 then
  ...
  ret._private.reboot_button:connect_signal(
    "button::press",
    function(c, _, _, button)
      if button == 1 then
  ...
  ret._private.poweroff_button:connect_signal(
    "button::press",
    function(c, _, _, button)
      if button == 1 then
