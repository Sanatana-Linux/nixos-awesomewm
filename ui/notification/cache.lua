...
local function ensure_cache_dir()
    ...
    if not success then
        io.stderr:write("Failed to create cache directory: " .. (err or "unknown error") .. "\n")
        return false
    end
    return true
end
...
local function load_cache()
    ...
    if success then
        ...
        io.stderr:write("Failed to parse notification cache: " .. tostring(result) .. "\n")
        return {}
    end
end
...
local function save_cache(notifications)
    ...
    if not success then
        io.stderr:write("Failed to encode notifications to JSON: " .. tostring(json_content) .. "\n")
        return false
    end
    
    local file = io.open(cache_file, "w")
    if not file then
        io.stderr:write("Failed to open cache file for writing: " .. cache_file .. "\n")
        return false
    end
