...
require('ui.bar.popups.notification_center')
-- -------------------------------------------------------------------------- --
...
screen.connect_signal('request::desktop_decoration', function(s)
	...
	local launcher_tooltip = utilities.make_popup_tooltip(
	...
	date:connect_signal('mouse::enter', function() awesome.emit_signal('calendar::visibility', true) end)

	date:connect_signal('mouse::leave', function() awesome.emit_signal('calendar::visibility', false) end)

	date:add_button(
		awful.button(
			{},
			1,
			function() clock.format = clock.format == clock_formats.hour and clock_formats.day or clock_formats.hour end
		)
	)
	-- -------------------------------------------------------------------------- --
	--                                 layout box                                 --
	...
	local function layoutname() return 'Layout: ' .. utilities.capitalize(awful.layout.get(s).name) end

	-- make custom tooltip for the whole button
	local layoutbox_tooltip = utilities.make_popup_tooltip(
		layoutname(),
		function(d)
			return awful.placement.bottom_right(d, {
				margins = {
	...
	local update_content = function() layoutbox_tooltip.widget.text = layoutname(),

	naughty.notification({
		title = 'Layout Changed',
		text = 'The current tag has had the client layout changed to the ' .. layoutbox_tooltip.widget.text .. ' layout.',
		image = gfs.get_configuration_dir() .. 'themes/icons/svg/awesome.png',
	})
	end
	...
	utilities.add_buttons(layoutbox, {
		awful.button({}, 1, function() awesome.emit_signal('layout::changed:next') end),
		awful.button({}, 3, function() awesome.emit_signal('layout::changed:prev') end),
	})
	...
	local animation = rubato.timed({
		...
		subscribed = function(pos) s.mywibox.y =  pos end,
	})
	...
	--
	s.activation_zone:connect_signal('mouse::enter', function() s.enable_wibox() end)
	-- ------------------------------------------------- --
	--  NOTE this keeps the bar open so long is the mouse is within its boundaries so the other can be hidden
	--
	s.mywibox:connect_signal('mouse::enter', function() s.detect:stop() end)
	-- ------------------------------------------------- --
	--  NOTE this keeps the bar open so long is the mouse is within its boundaries so the other can be hidden
	--
	s.mywibox:connect_signal('button::press', function() s.enable_wibox() end)
	-- ------------------------------------------------- --
	--  NOTE this keeps the bar open so long is the mouse is within its boundaries so the other can be hidden
	--
	s.mywibox:connect_signal('button::release', function() s.enable_wibox() end)
	-- ------------------------------------------------- --
	--  NOTE signals to begin timer when mouse leaves
	--
	s.mywibox:connect_signal('mouse::leave', function() s.detect:start() end)
