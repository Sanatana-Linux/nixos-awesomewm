...
local color = require(tostring(...):match(".*app_menu") .. ".helpers.color")
...
local function has_value(tab, val)
    for index, value in pairs(tab) do
        if val:find(case_insensitive_pattern(value)) then return true end
    end
    return false
end
...
local function create_app_widget(self, entry)
        ...
        if entry.terminal == true then
            if self.terminal ~= nil then
                local terminal_command =
                    terminal_commands_lookup[self.terminal] or self.terminal
    ...
    app:connect_signal("mouse::enter", function(_self)
        ...
        if app.selected then
            ...
            local hover_color = (is_dark or is_opaque) and
                                    color.rgba_to_hex(
                                        color.multiply(app_normal_color, 2.5)) or
                                    color.rgba_to_hex(
                                        color.multiply(app_normal_color, 0.5))
            app:get_children_by_id("background")[1].bg =
    ...
    app:connect_signal("button::press",
        ...
        if button == 1 then
            local app = _self
            if self._private.active_widget == app or
                not self.select_before_spawn then
                app.spawn()
            else
    ...
    if text == "" then
        ...
        for index, entry in pairs(self._private.all_entries) do
            ...
            if string.find(entry.name:lower(), text:lower(), 1, true) ~= nil or
                self.search_commands and
                string.find(entry.commandline, text:lower(), 1, true) ~= nil then
...
local function page_backward(self, direction)
    ...
    elseif self.wrap_page_scrolling and #self._private.matched_entries >=
        self._private.max_apps_per_page then
    ...
    elseif self.wrap_app_scrolling then
        ...
        unselect_app(self)
        select_app(self, math.min(rows, #self._private.grid.children %
                                      self.apps_per_row), columns)
        return
    else
        return
    end
    ...
    for index, entry in pairs(self._private.matched_entries) do
        ...
        if index > min_app_index_to_include and index <=
            max_app_index_to_include then
            self._private.grid:add(create_app_widget(self, entry))
        end
    end
    ...
    elseif self.wrap_page_scrolling then
        if direction == "up" then
            ...
            -- Keep the same row from last page
            select_app(self, math.min(pos.row, #self._private.grid.children %
                                          self.apps_per_row), columns)
        end
    end
end
...
local function page_forward(self, direction)
    ...
    if self._private.current_page > 1 or self.wrap_page_scrolling then
        if direction == "down" then
            ...
            local last_col_max_row = math.min(pos.row, #self._private.grid
                                                  .children % self.apps_per_row)
...
local function generate_apps(self)
    ...
    elseif self.reverse_sort_alphabetically then
        ...
        table.sort(apps, function(a, b)
            local app_a_favorite = has_value(self.favorites,
                                             app_info.get_name(a))
            local app_b_favorite = has_value(self.favorites,
                                             app_info.get_name(b))
    ...
    for _, app in ipairs(apps) do
        if app.should_show(app) then
            ...
            if not has_value(self.skip_names, name) and
                not has_value(self.skip_commands, commandline) then
                ...
                if icon ~= "" or self.skip_empty_icons == false then
                    ...
                    local terminal = Gio.DesktopAppInfo.get_string(
                                         desktop_app_info, "Terminal") == "true" and
                                         true or false
                    local generic_name =
                        Gio.DesktopAppInfo.get_string(desktop_app_info,
                                                      "GenericName") or nil
...
local function new(args)
    ...
    args.reverse_sort_alphabetically =
        args.reverse_sort_alphabetically ~= nil and
    ...
    args.hide_on_left_clicked_outside =
        args.hide_on_left_clicked_outside == nil and true or
            args.hide_on_left_clicked_outside
    args.hide_on_right_clicked_outside =
        args.hide_on_right_clicked_outside == nil and true or
            args.hide_on_right_clicked_outside
    args.hide_on_launch = args.hide_on_launch == nil and true or
                              args.hide_on_launch
    args.try_to_keep_index_after_searching =
        args.try_to_keep_index_after_searching ~= nil and
            args.try_to_keep_index_after_searching or false
    args.reset_on_hide = args.reset_on_hide == nil and true or
                             args.reset_on_hide
    ...
    args.prompt_border_width = args.prompt_border_width or
                                   beautiful.border_width or dpi(0)
    args.prompt_border_color = args.prompt_border_color or
                                   beautiful.border_color or args.prompt_color
    ...
    args.prompt_icon_font = args.prompt_icon_font or beautiful.nerd_font
    ...
    args.prompt_font = args.prompt_font or beautiful.nerd_font
    args.prompt_text_color = args.prompt_text_color or beautiful.bg_normal or
                                 "#000000"
    args.prompt_cursor_color =
        args.prompt_cursor_color or beautiful.bg_normal or "#000000"
    ...
    args.app_normal_hover_color = args.app_normal_hover_color or
                                      (color.is_dark(args.app_normal_color) or
                                          color.is_opaque(args.app_normal_color)) and
                                      color.rgba_to_hex(
                                          color.multiply(
                                              color.hex_to_rgba(
                                                  args.app_normal_color), 2.5)) or
                                      color.rgba_to_hex(
                                          color.multiply(
                                              color.hex_to_rgba(
                                                  args.app_normal_color), 0.5))
    ...
    args.app_selected_hover_color = args.app_selected_hover_color or
                                        (color.is_dark(args.app_normal_color) or
                                            color.is_opaque(
                                                args.app_normal_color)) and
                                        color.rgba_to_hex(
                                            color.multiply(
                                                color.hex_to_rgba(
                                                    args.app_selected_color),
                                                2.5)) or color.rgba_to_hex(
                                        color.multiply(
                                            color.hex_to_rgba(
                                                args.app_selected_color), 0.5))
    args.app_content_padding = args.app_content_padding or dpi(10)
    args.app_content_spacing = args.app_content_spacing or dpi(10)
    args.app_show_icon = args.app_show_icon == nil and true or
                             args.app_show_icon
    ...
    args.app_show_name = args.app_show_name == nil and true or
                             args.app_show_name
    args.app_name_generic_name_spacing =
        args.app_name_generic_name_spacing or dpi(0)
    args.app_name_halign = args.app_name_halign or "center"
    args.app_name_font = args.app_name_font or beautiful.nerd_font
    ...
    ret._private.prompt = prompt {
        ...
        history_path = ret.save_history == true and gfilesystem.get_cache_dir() ..
            "/history" or nil,
        changed_callback = function(text)
            if text == ret._private.text then return end

            if ret._private.search_timer ~= nil and
                ret._private.search_timer.started then
            ...
            ret._private.search_timer = gtimer {
                ...
                autostart = true,
                single_shot = true,
                callback = function() search(ret, text) end
            }

            ret._private.text = text
        end,
        keypressed_callback = function(mod, key, cmd)
            if key == "Escape" then ret:hide() end
            if key == "Return" then
    ...
    -- -------------------------------------------------------------------------- --
    -- grid widget (gotta set the scroll somewhere)
    ret._private.grid = wibox.widget {
        ...
        buttons = {
            ...
            -- Because the scroll wheel is a little wild sometimes
            awful.button({}, 8, function() scroll_up(ret) end),
            awful.button({}, 9, function() scroll_down(ret) end)
        }
    }
    -- -------------------------------------------------------------------------- --
    -- logo button
    ...
    -- Saving this for the off chance I figure out something useful to do with this
    -- ret._private.action_button:connect_signal("mouse::enter", function(c)
    --     c:set_bg(beautiful.widget_back_focus)
    --     c:set_fg(beautiful.lessgrey)
    -- end)
    -- ret._private.action_button:connect_signal("mouse::leave", function(c)
    --     c:set_bg(beautiful.widget_back)
    --     c:set_fg(beautiful.fg_normal)
    -- end)
    -- ret._private.action_button:connect_signal("button::press",
    --                                           function(c, _, _, button)
    --     if button == 1 then awesome.emit_signal("notification_center::toggle") end
    -- end)
    -- -------------------------------------------------------------------------- --
    -- log out (yes I jumped on the doas bandwagon, its more ergonomic to type)
    ret._private.exit_button = wibox.widget {
        widget = wibox.container.background,
    ...
    ret._private.exit_button:connect_signal("mouse::leave", function(c)
        c:set_bg(beautiful.widget_back)
        c:set_fg(beautiful.fg_normal)
    end)
    ...
    -- -------------------------------------------------------------------------- --
    -- reboot button
    ret._private.reboot_button = wibox.widget {
        widget = wibox.container.background,
        fg = beautiful.fg_normal,
        bg = beautiful.widget_back,
        shape = utilities.mkroundedrect(),
    ...
    ret._private.reboot_button:connect_signal("mouse::leave", function(c)
        c:set_bg(beautiful.widget_back)
        c:set_fg(beautiful.fg_normal)
    end)
    ...
    -- -------------------------------------------------------------------------- --
    -- power off 
    ret._private.poweroff_button = wibox.widget {
        widget = wibox.container.background,
        fg = beautiful.fg_normal,
        bg = beautiful.widget_back,
        shape = utilities.mkroundedrect(),
    ...
    ret._private.poweroff_button:connect_signal("mouse::leave", function(c)
        c:set_bg(beautiful.widget_back)
        c:set_fg(beautiful.fg_normal)
    end)
            ...
            {{
                {
                    layout = wibox.layout.align.vertical,
                    {
                        ...
                        spacing = 0,
                        ret._private.action_button
                    },
                    nil,
                    {
                        layout = wibox.layout.fixed.vertical,
                        spacing = dpi(6),
                        ret._private.exit_button,
                        ret._private.reboot_button,
                        ret._private.poweroff_button
                    }
                },
            widget=wibox.container.margin,
            margins=dpi(8),
    ...
    if ret.hide_on_left_clicked_outside then
        awful.mouse.append_client_mousebinding(
            awful.button({}, 1, function(c) ret:hide() end))
    ...
    if ret.hide_on_right_clicked_outside then
        awful.mouse.append_client_mousebinding(
            awful.button({}, 3, function(c) ret:hide() end))
    ...
    awful.spawn.easy_async_with_shell(kill_old_inotify_process_script,
                                      function()
        awful.spawn.with_line_callback(subscribe_script, {
            stdout = function(_) generate_apps(ret) end
        })
    end)
