...
local function select_app(self, x, y)
	...
	if widgets then
		...
		if self._private.active_widget ~= nil then
			...
			if name_widget then
				name_widget.markup =
					string.format("<span foreground='%s'>%s</span>", beautiful.lessgrey, name_widget.text)
			end
		end
	end
end
...
local function create_app_widget(self, entry)
	...
	local app = wibox.widget({
		...
		bg = self.app_normal_color,
		{
			...
			margins = { top = 0, left = dpi(8), right = 0, bottom = 0 },
			{
	...
	app:connect_signal("mouse::enter", function(_self)
		...
		if app.selected then
			...
			app:get_children_by_id("background")[1].bg = beautiful.bg_normal
		end
	end)
...
local function page_backward(self, direction)
	...
	elseif self.wrap_app_scrolling then
		...
		unselect_app(self)
		select_app(self, math.min(rows, #self._private.grid.children % 9), columns)
		return
	else
		return
	end
	...
	elseif self.wrap_page_scrolling then
		if direction == "up" then
			...
			-- Keep the same row from last page
			select_app(self, math.min(pos.row, #self._private.grid.children % 9), columns)
		end
	end
end
		...
		if app.should_show(app) then
			...
			if not has_value(self.skip_names, name) and not has_value(self.skip_commands, commandline) then
				...
				if icon ~= "" or self.skip_empty_icons == false then
					...
					local terminal = Gio.DesktopAppInfo.get_string(desktop_app_info, "Terminal")
								== "true"
							and true
						or false
...
local slide_right = animations.instance.timed({
	...
	subscribed = function(pos)
		app_launcher.y = (screen.geometry.y - beautiful.bar_height) + pos
	end,
})
...
function app_launcher:show()
	...
	slide_right.target = screen.geometry.height - (screen.app_launcher.height + beautiful.useless_gap * 2)
...
function app_launcher:hide()
	...
	slide_right.target = screen.geometry.height
...
local function new(args)
	...
	args.prompt_color = beautiful.bg_lighter .. 'aa'
	...
	args.prompt_text_halign = "left"
	args.prompt_text_valign =  "center"
	...
	args.prompt_icon_font = beautiful.nerd_font
	...
	args.prompt_text_color = beautiful.fg_normal

	args.prompt_cursor_color = beautiful.fg_normal

	args.apps_per_row = 9
	args.apps_per_column = 1
	args.apps_margin = { top = 0, left = dpi(0), right = 0, bottom = 0 }
	args.apps_spacing = 0

	args.expand_apps = args.expand_apps == nil and true or args.expand_apps
	args.app_width = dpi(300)
	args.app_height = dpi(60)
	args.app_shape = args.app_shape or nil
	args.app_normal_color = args.app_normal_color or beautiful.bg_normal or "#000000"
	args.app_normal_hover_color = beautiful.bg_normal
	...
	args.app_content_padding = { top = 0, left = dpi(8), right = 0, bottom = 0 }
	args.app_content_spacing = dpi(10)
	args.app_show_icon = args.app_show_icon == nil and true or args.app_show_icon
	args.app_icon_halign = "left"
	args.app_icon_width = dpi(36)
	args.app_icon_height = dpi(36)
	args.app_show_name = true or args.app_show_name
	args.app_name_generic_name_spacing = args.app_name_generic_name_spacing or dpi(0)
	args.app_name_halign = "center"
	...
	args.app_name_selected_color = beautiful.lessgrey
	...
	local grid_width = dpi(300) or nil
	local grid_height = ret.shrink_height == false and dpi(540) or nil
	-- -------------------------------------------------------------------------- --
	-- Prompt Widget
	--
	...
	ret._private.exit_button:connect_signal("button::press", function(c, _, _, button)
		if button == 1 then
	...
	ret._private.reboot_button:connect_signal("button::press", function(c, _, _, button)
		if button == 1 then
			awful.spawn.with_shell("doas reboot -f")
		end
	end)
	-- -------------------------------------------------------------------------- --
	-- power off
	...
	ret._private.poweroff_button:connect_signal("button::press", function(c, _, _, button)
		if button == 1 then
			awful.spawn.with_shell("doas poweroff -f")
		end
	end)
	-- -------------------------------------------------------------------------- --
		...
		widget = {
			...
			},
			{
				...
				},
				{
					{
						--  require(
						--    "ui.bar.popups.app_menu.widget.app_launcher.music-player"
						-- ),
						widget = wibox.container.background,
	...
	ret._private.max_apps_per_page = 1 * 9
