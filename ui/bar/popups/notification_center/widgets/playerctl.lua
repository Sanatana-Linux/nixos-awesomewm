...
widget:connect_signal('start_updating', function() currently_updating = true end)

widget:connect_signal('stop_updating', function() currently_updating = false end)
...
local titlefont, artistfont, margin, basewidth = beautiful.nerd_font .. ' 12', beautiful.font .. ' 10', dpi(5), dpi(430)
			...
			bottom = dpi(5),
			{
				...
				},
				{
					...
					forced_height = insideheight,
					{
						...
						{
							widget = wibox.container.margin,
							margins = { left = 2 * margin, bottom = margin, right = margin },
							{
								layout = wibox.layout.fixed.vertical,
								spacing = margin,
								{
						...
						margins = { left = 2 * margin, right = 2 * margin },
						{
							...
							},
							{
								...
								valign = 'center',
								{
									...
									forced_height = dpi(5),
									max_value = 100,
									forced_width = basewidth - 22 * margin - dpi(80),
									border_width = 0,
...
local function image_with_gradient(image, ratio)
	...
	local surf = utilities.crop_surface(ratio, in_surf)
	...
	pat_h:add_color_stop_rgba(0.6, gears.color.parse_color(beautiful.dark_grey .. 'BB'))
	pat_h:add_color_stop_rgba(0.8, gears.color.parse_color(beautiful.dark_grey .. '99'))
	pat_h:add_color_stop_rgba(1, gears.color.parse_color(beautiful.dark_grey .. '88'))
	cr:set_source(pat_h)
	cr:fill()
...
local function update_widget_meta(w, meta, player)
	...
	if length then w:get_children_by_id('progress')[1].max_value = math.floor(length / 1000000) end
	...
	if art then
		if string.match(art, 'file://') then
			...
			path = unescape(path)
			if gears.filesystem.file_readable(path) then set_bg_with_gradient(w, path) end
		elseif player.player_name == 'spotify' then
			...
			if gears.filesystem.file_readable(path) then
				...
				awful.spawn.easy_async('curl -L -s ' .. art .. ' -o ' .. path, function(_, _, _, code)
					-- check if download was successful
					if code == 0 then set_bg_with_gradient(w, path) end
				end)
			end
		end
	end
...
local function widget_from_player(player)
	...
	player.on_pause = function(_, _)
		w:get_children_by_id('playpause')[1]:set_image(gcl.recolor_image(maticons .. 'play.svg', beautiful.bg_normal))
		w:get_children_by_id('playpause_bg')[1].bg = beautiful.grey
		progresstimer:stop()
	end

	player.on_play = function(_, _)
		w:get_children_by_id('playpause')[1]:set_image(gcl.recolor_image(maticons .. 'pause.svg', beautiful.bg_normal))
		w:get_children_by_id('playpause_bg')[1].bg = beautiful.blue
		progresstimer_run()
	end

	player.on_metadata = function(_, meta, _) update_widget_meta(w, meta, player) end

	player.on_shuffle = function(_, shuffle, _)
		w:get_children_by_id('shuffle')[1]
			:set_image(gcl.recolor_image(maticons .. 'shuffle.svg', shuffle and active_color or inactive_color))
	end

	player.on_loop_status = function(_, status, _)
		w:get_children_by_id('repeat')[1]
			:set_image(gcl.recolor_image(maticons .. 'repeat.svg', status == 'NONE' and inactive_color or active_color))
	end

	func_on_click(w:get_children_by_id('prev')[1], function() player:previous() end)
	func_on_click(w:get_children_by_id('playpause_bg')[1], function() player:play_pause() end)
	...
	func_on_click(
		w:get_children_by_id('repeat')[1],
		function() player:set_loop_status(player.loop_status == 'NONE' and 'PLAYLIST' or 'NONE') end
	)

	widget:connect_signal('start_updating', function()
		if player.playback_status == 'PLAYING' then progresstimer_run() end
	end)
	widget:connect_signal('stop_updating', function() progresstimer:stop() end)
	----
	-- deconstruct
	----
	...
	w:get_children_by_id('playpause')[1]:set_image(
		gcl.recolor_image(
			maticons .. (player.playback_status == 'PLAYING' and 'pause.svg' or 'play.svg'),
			beautiful.bg_normal
		)
	)
	...
	w:get_children_by_id('shuffle')[1]
		:set_image(gcl.recolor_image(maticons .. 'shuffle.svg', player.shuffle and active_color or inactive_color))
...
function manager:on_name_appeared(name, _) start_managing(name, true) end

for _, name in ipairs(manager.player_names) do
	start_managing(name, false)
end

function widget:disable_updates() widget:emit_signal('stop_updating') end

function widget:enable_updates() widget:emit_signal('start_updating') end
