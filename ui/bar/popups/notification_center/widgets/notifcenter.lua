local iconsdir = gears.filesystem.get_configuration_dir() ..
                     "themes/assets/icons/svg/"

local function cross_enter(self, _)
  self:get_children_by_id("remove")[1]:set_image(
      gears.color.recolor_image(iconsdir .. "close.svg", beautiful.red))
end
local function cross_leave(self, _)
  self:get_children_by_id("remove")[1]:set_image(
      gears.color.recolor_image(iconsdir .. "close.svg", beautiful.fg_normal))
end
-- -------------------------------------------------------------------------- --
local main_widget
        ...
        forced_height = beautiful.get_font_height(beautiful.nerd_font .. " 11"),
        {
          ...
          valign = "bottom",
          {
            ...
            forced_height = beautiful.get_font_height(
                beautiful.nerd_font .. " 11") / 2,
            forced_width = beautiful.get_font_height(
                beautiful.nerd_font .. " 11")
          }
        }
      },
      {
      ...
      bg = beautiful.bg_lighter .. "88",
      {
        ...
        margins = dpi(5),
        {
          ...
          expand = "inside",
          {
            id = "reveal_button",
            widget = wibox.widget.imagebox,
            image = gears.color.recolor_image(iconsdir .. "expand_more.svg",
                                              beautiful.fg_normal)
          },
          {
            widget = wibox.container.place,
          ...
          },
          {
            id = "clear_button",
            widget = wibox.widget.imagebox,
            image = gears.color.recolor_image(iconsdir .. "clear_all.svg",
                                              beautiful.fg_normal)
          }
        }
      }
...
local function add_notif_widget(n)
    ...
    drawer:get_children_by_id("reveal_button")[1]:add_button(awful.button {
      ...
      on_press = function()
        if drawer.collapsed then
          drawer:get_children_by_id("notifs")[1]:set_children(
              drawer.attached_notifs)
          drawer:get_children_by_id("notifs_margin")[1].margins = dpi(5)
          drawer:get_children_by_id("reveal_button")[1]:set_image(gears.color
                                                                      .recolor_image(
                                                                      iconsdir ..
                                                                          "expand_less.svg",
                                                                      beautiful.fg_normal))
  ...
  w:get_children_by_id("remove")[1]:add_button(awful.button {
    ...
    button = 1,
    on_press = function()
      w:get_children_by_id("remove")[1]:disconnect_signal("mouse::enter",
                                                          cross_enter)
      w:get_children_by_id("remove")[1]:disconnect_signal("mouse::leave",
                                                          cross_leave)
      ...
      drawer:get_children_by_id("appname")[1].text =
          drawer.app .. " (" .. #drawer.attached_notifs .. ")"
      ...
      spacing = dpi(5),
      {
        ...
        shape = utilities.mkroundedrect(),
        {
          ...
          nil,
          {
            ...
            {
              widget = wibox.container.constraint,
              height = beautiful.get_font_height(beautiful.nerd_font .. " 18") +
                  dpi(10),
              {
                widget = wibox.container.margin,
                margins = dpi(5),
