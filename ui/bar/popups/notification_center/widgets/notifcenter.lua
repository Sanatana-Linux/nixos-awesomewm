local iconsdir = gears.filesystem.get_configuration_dir() .. 'themes/assets/icons/svg/'

local function cross_enter(self, _)
	self:get_children_by_id('remove')[1]:set_image(gears.color.recolor_image(icons.close, beautiful.red))
end
	...
	shape = utilities.mkroundedrect(),
	{
		...
		strategy = 'max',
		{
			layout = wibox.layout.fixed.vertical,
			{
				...
				forced_height = beautiful.get_font_height(beautiful.nerd_font .. ' 11'),
				{
					...
					valign = 'bottom',
					{
						...
						forced_height = beautiful.get_font_height(beautiful.nerd_font .. ' 11') / 2,
						forced_width = beautiful.get_font_height(beautiful.nerd_font .. ' 11'),
					},
				},
			},
			{
	...
	border_width = dpi(1.25),
	{
			...
			bg = beautiful.bg_contrast .. '88',
			{
				...
				margins = dpi(5),
				{
					...
					expand = 'inside',
					{
						id = 'reveal_button',
						widget = wibox.widget.imagebox,
						image = gears.color.recolor_image(icons.arrow_down, beautiful.fg_normal),
					},
					{
						widget = wibox.container.place,
						halign = 'center',
					...
					},
					{
						id = 'clear_button',
						widget = wibox.widget.imagebox,
						image = gears.color.recolor_image(icons.clear_all, beautiful.fg_normal),
					},
				},
			},
		},
		{
			...
			bg = beautiful.bg_contrast .. '88',
			{
				...
				{ id = 'notifs', layout = wibox.layout.fixed.vertical, spacing = dpi(3) },
			},
		},
	},
}
...
local function add_notif_widget(n)
	...
	if not drawer then
		...
		drawer:get_children_by_id('reveal_button')[1]:add_button(awful.button({
			...
			on_press = function()
				if drawer.collapsed then
					...
					drawer
						:get_children_by_id('reveal_button')[1]
						:set_image(gears.color.recolor_image(icons.arrow_down, beautiful.fg_normal))
				else
					drawer:get_children_by_id('notifs')[1]:reset()
					drawer:get_children_by_id('notifs_margin')[1].margins = 0
					drawer
						:get_children_by_id('reveal_button')[1]
						:set_image(gears.color.recolor_image(icons.arrow_up, beautiful.fg_normal))
				end
				drawer.collapsed = not drawer.collapsed
			end,
		}))
		utilities.pointer_on_focus(drawer:get_children_by_id('reveal_button')[1])
		...
		utilities.pointer_on_focus(drawer:get_children_by_id('clear_button')[1])
		main_widget:insert(1, drawer)
	end
	-- -------------------------------------------------------------------------- --
	-- create notification widget and add it to its widget's drawer
	--
	w:get_children_by_id('remove')[1]:set_image(gears.color.recolor_image(icons.close, beautiful.fg_normal))
	...
	drawer:get_children_by_id('appname')[1].text = drawer.app .. ' (' .. #drawer.attached_notifs .. ')'
	if not drawer.collapsed then drawer:get_children_by_id('notifs')[1]:insert(1, w) end
end
-- -------------------------------------------------------------------------- --
local notifbox
	...
	spacing = dpi(5),
	{
		...
		shape = utilities.mkroundedrect(),
		{
			...
			nil,
			{
				...
				{
					widget = wibox.container.constraint,
					height = beautiful.get_font_height(beautiful.nerd_font .. ' 18') + dpi(10),
					{
						widget = wibox.container.margin,
						margins = dpi(5),
						{
...
naughty.connect_signal('request::display', function(n)
	...
	if client.focus then client_focused = string.lower(client.focus.class) ~= (n.app_name or '') end
	add_notif_widget(n)
end)
-- -------------------------------------------------------------------------- --
client.connect_signal('property::active', function(c)
	...
	if c then
		...
		for _, entry in ipairs(main_widget:get_children()) do
			if string.lower(entry.app) == cname then drawer = entry end
		end
		main_widget:remove_widgets(drawer)
	end
end)
