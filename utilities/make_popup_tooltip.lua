...
local mkroundedcontainer = require('utilities.mkroundedcontainer')
...
return function(text, placement)
	...
	ret.widget = wibox.widget({
		...
		set_text = function(self, t) self:get_children_by_id('text')[1].markup = t end,
		set_image = function(self, i) self:get_children_by_id('image')[1].image = i end,
	})
	...
	function ret.hide() self.visible = false end

	function ret.toggle()
		if not self.visible and self.screen ~= awful.screen.focused() then self.screen = awful.screen.focused() end
