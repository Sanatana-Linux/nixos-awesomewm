...
function common.create_buttons(buttons, object)
    local is_formatted = buttons
            and buttons[1]
            and (type(buttons[1]) == "button" or buttons[1]._is_capi_button)
        or false
    ...
    if buttons then
        ...
        for _, src in ipairs(buttons) do
            ...
            for _, b in ipairs(is_formatted and { src } or src) do
                ...
                -- argument.
                local btn =
                    capi.button({ modifiers = b.modifiers, button = b.button })
...
local function custom_template(args)
    ...
    assert(
        ...
        "The given widget template did not result in a"
...
function common.list_update(w, buttons, label, data, objects, args)
    ...
    for i, o in ipairs(objects) do
        ...
        if not cache then
            cache = (args and args.widget_template) and custom_template(args)
                or default_template()
