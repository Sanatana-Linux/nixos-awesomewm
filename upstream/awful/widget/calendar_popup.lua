...
local function parse_cell_options(cell, args)
    ...
    for _, prop in ipairs(properties) do
        ...
        if prop == "fg_color" then
            default = cell == "focus" and beautiful.fg_focus
                or beautiful.fg_normal
        elseif prop == "bg_color" then
            default = cell == "focus" and beautiful.bg_focus
        ...
        props[prop] = args[prop]
            or beautiful["calendar_" .. cell .. "_" .. prop]
            or bl_style[prop]
            or default
    end
    if cell == "focus" and props.markup == nil then
        local fg = props.fg_color
                and string.format(
                    'foreground="%s"',
                    gears.color.to_rgba_string(props.fg_color)
                )
            or ""
        local bg = props.bg_color
                and string.format(
                    'background="%s"',
                    gears.color.to_rgba_string(props.bg_color)
                )
            or ""
        props.markup =
            string.format("<span %s %s><b>%s</b></span>", fg, bg, "%s")
    end
    return props
end
...
local function get_geometry(widget, screen, position)
    ...
    local wa = s.workarea
    local width, height =
        widget:fit({ screen = s, dpi = s.dpi }, wa.width, wa.height)
...
function calendar_popup:call_calendar(offset, position, screen)
    ...
    local date =
        { day = raw_date.day, month = raw_date.month, year = raw_date.year }
    if widget._private.type == "month" and self.offset ~= 0 then
        local month_offset = (raw_date.month + self.offset - 1) % 12 + 1
        local year_offset = raw_date.year
            + math.floor((raw_date.month + self.offset - 1) / 12)
...
local function get_cal_wibox(caltype, args)
    ...
    local widget = wibox.widget({
        ...
        widget = caltype == "year" and wibox.widget.calendar.year
            or wibox.widget.calendar.month,
    })
