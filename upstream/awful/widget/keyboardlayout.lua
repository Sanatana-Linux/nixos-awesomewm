...
function keyboardlayout.get_groups_from_group_names(group_names)
    ...
    local pattern_and_callback_pairs = {
        ...
        ["^" .. word_pat .. "/" .. word_pat .. sec_pat .. idx_pat .. "$"] = function(
            token,
            pattern
        )
            local vendor, file, section, group_idx =
                string.match(token, pattern)
            return vendor, file, section, group_idx
        end,
        -- vendor/file(section)
        ["^" .. word_pat .. "/" .. word_pat .. sec_pat .. "$"] = function(
            token,
            pattern
        )
        ...
        ["^" .. word_pat .. "/" .. word_pat .. idx_pat .. "$"] = function(
            token,
            pattern
        )
        ...
        ["^" .. word_pat .. sec_pat .. idx_pat .. "$"] = function(
            token,
            pattern
        )
...
local function update_layout(self)
    self._layout = {}
    local layouts = keyboardlayout.get_groups_from_group_names(
...
function keyboardlayout.new(args)
    ...
    local self =
        widget_base.make_widget(widget, nil, { enable_properties = true })
