...
function markup.fg(color, text)
    return '<span foreground="'
        .. rgba(color, beautiful.fg_normal)
        .. '">'
        .. tostring(text)
        .. "</span>"
end
...
function markup.bg(color, text)
    return '<span background="'
        .. rgba(color, beautiful.bg_normal)
        .. '">'
        .. tostring(text)
        .. "</span>"
end
...
function widget.new(args)
    ...
    for k, v in pairs(awful.key.keygroups) do
        ...
        for k2, v2 in pairs(v) do
            ...
            widget_instance._keygroups[k][k2] = widget_instance.labels[keysym]
                or keyprint
                or keysym
                or v2[1]
        end
    ...
    function widget_instance:_load_widget_settings()
        ...
        self.border_width = args.border_width
            or beautiful.hotkeys_border_width
            or beautiful.border_width
        self.border_color = args.border_color
            or beautiful.hotkeys_border_color
            or self.fg
        self.shape = args.shape or beautiful.hotkeys_shape
        self.modifiers_fg = args.modifiers_fg
            or beautiful.hotkeys_modifiers_fg
        ...
        self.override_label_bgs = args.override_label_bgs
            or beautiful.hotkeys_override_label_bgs
            or false
        ...
        self.description_font = args.description_font
            or beautiful.hotkeys_description_font
            or "Monospace 8"
        self.group_margin = args.group_margin
            or beautiful.hotkeys_group_margin
            or dpi(6)
    ...
    function widget_instance:_get_next_color(id)
        ...
        if self._colors_counter[id] then
            self._colors_counter[id] = math.fmod(
        ...
        return self.label_colors["color" .. tostring(
            self._colors_counter[id],
            15
        )]
    end
        ...
        for _, keys in pairs(self._cached_awful_keys) do
            for _, params in pairs(keys) do
                if #params.keylist > 4 then
                    ...
                    local keygroup = gtable.find_first_key(
                        self._keygroups,
                        function(_, v)
    ...
    function widget_instance:_group_label(group, color)
        ...
        if not bg_color then
            if self.override_label_bgs then
                ...
                bg_color = self.group_rules[group]
                        and self.group_rules[group].color
    ...
    function widget_instance:_create_group_columns(
        column_layouts,
        group,
        keys,
        s,
        ...
        -- +1 for group label:
        local items_height = gstring.linecount(joined_descriptions)
                * line_height
            + group_label_height
        ...
        for i, column in ipairs(column_layouts) do
            if
                ...
                or (
                    i == #column_layouts
                    and column.height_px < max_height_px / 2
                )
            then
        ...
        if items_height > available_height_px then
            ...
            local available_height_items = (
                available_height_px - group_label_height * 2
            ) / line_height
            for i = 1, #keys do
                table.insert(
                    (
                        (i < available_height_items) and new_keys
                        or overlap_leftovers
                    ),
                    keys[i]
                )
            end
        ...
        local function insert_keys(ik_keys, ik_add_new_column)
            ...
            for i, key in ipairs(ik_keys) do
                ...
                if key.keylist and #key.keylist > 1 then
                    for each_key_idx, each_key in ipairs(key.keylist) do
                        ...
                        if each_key_idx ~= #key.keylist then
                            key_label = key_label
                                .. markup.fg(self.modifiers_fg, "/")
                        end
                    end
                elseif key.key then
                    key_label = gstring.xml_escape(key.key)
                end
                local rendered_hotkey = markup.font(
                    self.font,
                    modifiers .. key_label .. " "
                ) .. markup.font(
                ...
                local label_width = wibox.widget.textbox.get_markup_geometry(
                    rendered_hotkey,
                    s
                ).width
            ...
            if
                not current_column.max_width
                or (max_width > current_column.max_width)
            then
    ...
    function widget_instance:_create_wibox(
        ...
        s = get_screen(s)
        local wa = s.workarea
        local wibox_height = (self.height < wa.height) and self.height
            or (wa.height - self.border_width * 2)
        local wibox_width = (self.width < wa.width) and self.width
            or (wa.width - self.border_width * 2)
        ...
        for _, item in ipairs(column_layouts) do
            if item.max_width > available_width_px then
                ...
                item.layout:insert(
                    1,
                    self:_group_label("PgUp - Prev Page", self.label_bg)
                )
            else
    ...
    function widget_instance:show_help(c, s, show_args)
        ...
        local joined_groups = join_plus_sort(available_groups)
            .. tostring(show_awesome_keys)
        if not self._cached_wiboxes[s] then
            self._cached_wiboxes[s] = {}
        end
        if not self._cached_wiboxes[s][joined_groups] then
            self._cached_wiboxes[s][joined_groups] =
                self:_create_wibox(s, available_groups, show_awesome_keys)
        end
