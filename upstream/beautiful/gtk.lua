...
local function read_gtk_color_properties_from_widget(gtk_widget, properties)
    ...
    for result_key, style_context_property in pairs(properties) do
        result[result_key] =
            get_gtk_color_property_to_hex(style_context, style_context_property)
    end
    return result
end
...
function gtk.get_theme_variables()
    ...
    result = join(
        result,
        read_gtk_color_properties_from_widget(window, {
    ...
    result["font_size"] = get_gtk_property(label_style_context, "font-size")
        * gdk_scale
        / xrdb_scale
        / pt_to_px_ratio
    result["font_family"] =
        get_gtk_property(label_style_context, "font-family")[1]

    result = join(
        result,
        read_gtk_color_properties_from_widget(Gtk.Entry(), {
    ...
    result = join(
        result,
        read_gtk_color_properties_from_widget(Gtk.ToggleButton(), {
    ...
    result = join(
        result,
        read_gtk_color_properties_from_widget(button, {
    ...
    for result_key, style_context_property in pairs({
        ...
        local state = style_context:get_state()
        local property =
            button_style_context:get_property(style_context_property, state)
    ...
    result = join(
        result,
        read_gtk_color_properties_from_widget(headerbar, {
    ...
    if result.menubar_bg_color and result.menubar_bg_color ~= "#00000000" then
        ...
        result = join(
            result,
            read_gtk_color_properties_from_widget(label, {
    ...
    if
        result.header_button_bg_color
        and result.header_button_bg_color ~= "#00000000"
    then
        result = join(
            result,
            read_gtk_color_properties_from_widget(button, {
    ...
    result = join(
        result,
        read_gtk_color_properties_from_widget(error_button, {
    ...
    for _, color_data in ipairs({
        ...
        local result_key, style_context_key, fallback_key, fallback_key2 =
            unpack(color_data)
        result[result_key] = lookup_gtk_color_to_hex(
            style_context,
            style_context_key
        )
            or (
                result[result_key] ~= "#00000000" and result[result_key]
                or result[fallback_key]
                or result[fallback_key2]
                or result[result_key] -- <-- here is for case if it was meant to be a fully transparent color on purpose
            )
