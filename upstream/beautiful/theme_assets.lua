...
local function make_letter(cr, n, lines, size, bg, fg, alt_fg)
    ...
    cr:rectangle(0, (size + letter_gap) * n, size, size)
    cr:fill()
...
function theme_assets.recolor_titlebar(
    theme,
    color,
    state,
    postfix,
    ...
    local titlebar_icon_names = toggle_state and {}
        or {
            "titlebar_close_button_" .. state,
            "titlebar_minimize_button_" .. state,
        }
    for _, ts in
        ipairs(toggle_state and { toggle_state } or { "_active", "_inactive" })
    do
        table.insert(
            titlebar_icon_names,
            "titlebar_ontop_button_" .. state .. ts
        )
        table.insert(
            titlebar_icon_names,
            "titlebar_sticky_button_" .. state .. ts
        )
        table.insert(
            titlebar_icon_names,
            "titlebar_floating_button_" .. state .. ts
        )
        table.insert(
            titlebar_icon_names,
            "titlebar_maximized_button_" .. state .. ts
        )
    end
    for _, titlebar_icon_name in ipairs(titlebar_icon_names) do
        local full_name = postfix
                and (titlebar_icon_name .. postfix)
