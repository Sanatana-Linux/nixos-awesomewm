...
function module.arc(
    ...
    if end_rounded then
        ...
        end_angle = start_angle
            + math.max(0, arc_length / (radius - thickness / 2))
    end
    ...
    if start_rounded then
        ...
        local median_angle =
            atan2(start_p2[1] - start_p1[1], -(start_p2[2] - start_p1[2]))
...
function module.solid_rectangle_shadow(cr, w, h, x_offset, y_offset)
    ...
    local rect2 = { x0 = x_offset, y0 = y_offset, x1 = w + x_offset, y1 = h
        + y_offset }
...
function module.radial_progress(cr, w, h, percent, hide_left)
    ...
    if percent >= bar_percent + arc_percent then
        ...
        cr:stroke()
    elseif
        percent > bar_percent and percent < bar_percent + (arc_percent / 2)
    then
        cr:arc(
            w - h / 2,
            h / 2,
            ...
            (math.pi / 2)
                - (
                    (math.pi / 2) * (
                        (percent - bar_percent) / (arc_percent / 2)
                    )
                ),
            math.pi / 2
        )
    ...
    elseif
        percent >= bar_percent + arc_percent / 2
        and percent < bar_percent + arc_percent
    then
        cr:arc(w - h / 2, h / 2, h / 2, 0, math.pi / 2)
        cr:stroke()
        local add = (math.pi / 2)
            * ((percent - bar_percent - arc_percent / 2) / (arc_percent / 2))
    ...
    if percent > 2 * bar_percent + arc_percent then
        ...
        cr:stroke()
    elseif
        percent > bar_percent + arc_percent
        and percent < 2 * bar_percent + arc_percent
    then
        cr:move_to((h / 2) + (w - h), 0)
        cr:line_to(
            ((h / 2) + (w - h))
                - total_length * (percent - bar_percent - arc_percent),
            0
        )
        cr:stroke()
    end
    ...
    if not hide_left then
        ...
        elseif percent > 2 * bar_percent + arc_percent then
            local relpercent = (percent - 2 * bar_percent - arc_percent)
                / arc_percent
            cr:arc(
                h / 2,
                h / 2,
                h / 2,
                3 * (math.pi / 2) - math.pi * relpercent,
                3 * (math.pi / 2)
            )
            cr:stroke()
        end
